...
export function createArenaEnemySystem({ THREE, worldRoot, materials = {}, arenaRadius = 18 } = {}){
  ...
  const projectiles = [];
  const deathPieces = [];
  const tuning = { playerHp:100, lastPlayerHit:'', heightScale:1.3, speedScale:.5, waveSize:6, idleRangeScale:3 };
  ...
  function damageEnemy(e, amount, knock = { x:0, z:0 }, opts = {}){
    ...
    const km=e.lastKnockMul || (e.hitStage>=3?1.85:(e.hitStage===2?1.25:.9)); e.knockX += (knock.x || 0) * km; e.knockZ += (knock.z || 0) * km;
    if(e.hp <= 0){
      kills++; waveKills++;
