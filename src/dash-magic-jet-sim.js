...
// Integration changes: grid dimensions come from the layout instead of the
// prototype's quality dropdown, and maze line segments build the solid mask
// instead of the prototype's hard-coded demo rectangles/circles.
export function createDashJetSimulation({settings,layout,getMazeSegments=()=>[]}={}){
  const PATCH_W=layout.patchWidth;
  const PATCH_D=layout.patchDepth;
  const simW=layout.simulationColumns;
  ...
  function project(){
    ...
    for(let k=0;k<pressureIters;k++)for(let y=1;y<simH-1;y++)for(let x=1;x<simW-1;x++){
      const i=idx(x,y);if(solid[i])continue;
  ...
  function buildSolidMask(){
    ...
    // Patch re-centering happens on every dash, so pre-filter the maze segment
    // list down to those whose inflated bounding box can reach the patch.
    const minWx=centerX-PATCH_W*.5-wallRadius,maxWx=centerX+PATCH_W*.5+wallRadius;
    const minWz=centerZ-PATCH_D*.5-wallRadius,maxWz=centerZ+PATCH_D*.5+wallRadius;
    const segments=(getMazeSegments?.()||[]).filter(segment=>{
      if(!segment?.a||!segment?.b)return false;
      const sMinX=Math.min(segment.a.x,segment.b.x),sMaxX=Math.max(segment.a.x,segment.b.x);
      const sMinZ=Math.min(segment.a.z,segment.b.z),sMaxZ=Math.max(segment.a.z,segment.b.z);
      return sMaxX>=minWx&&sMinX<=maxWx&&sMaxZ>=minWz&&sMinZ<=maxWz;
    });
