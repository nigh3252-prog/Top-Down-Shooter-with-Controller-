...
export function makeWaterPrisonAttachedVisual({THREE,scene,size=1,stableId='',renderMode='style',stackIndex=0}={}){
  ...
  const radius=1.35*size,markers={...WATER_PRISON_VISUAL_CONTRACT,renderMode,phase:'attached',carrierRadius:.34,attachedRadius:1.35,rims:0,patches:0,fragments:0,droplets:0};
  const shell=tag(new THREE.Mesh(new THREE.SphereGeometry(radius,28,18),material(THREE,WATER,.46,{additive:false,wireframe:renderMode==='proxy'})),'waterPrisonAttachedShell');shell.scale.set(1,.94,1);group.add(remember(shell));
  if(renderMode!=='proxy'){
    const inner=tag(new THREE.Mesh(new THREE.SphereGeometry(radius*.76,20,14),material(THREE,WATER_DEEP,.24,{additive:false})),'waterPrisonAttachedInner');inner.position.y=-.04*size;group.add(remember(inner));
    for(let index=0;index<3;index++){const rim=tag(new THREE.Mesh(new THREE.TorusGeometry(radius*(.76+index*.075),.045*size,8,42),material(THREE,index===1?WATER_WHITE:WATER_PALE,.72-index*.08)),'waterPrisonAttachedRim',index);rim.rotation.x=(index===0?Math.PI/2:.55+index*.44);rim.rotation.z=index*.78+stackIndex*.31;rim.userData.spin=(index%2?1:-1)*(1.1+index*.28);group.add(remember(rim));markers.rims++;}
    for(let index=0;index<6;index++){const angle=index*Math.PI/3,patch=tag(new THREE.Mesh(new THREE.DodecahedronGeometry((.17+(index%2)*.04)*size,0),material(THREE,index%3?WATER_PALE:WATER_WHITE,.62)),'waterPrisonShellPatch',index);patch.userData.angle=angle;patch.userData.radius=radius*(.76+(index%2)*.08);patch.userData.speed=(index%2?1:-1)*(.55+index*.08);patch.position.set(Math.cos(angle)*patch.userData.radius,(.28*Math.sin(angle*1.7))*size,Math.sin(angle)*patch.userData.radius);patch.scale.set(1.8,.48,.72);group.add(remember(patch));markers.patches++;}
    for(let index=0;index<7;index++){const angle=index*Math.PI*2/7,fragment=tag(new THREE.Mesh(new THREE.DodecahedronGeometry((.12+(index%3)*.025)*size,0),material(THREE,index%2?WATER:WATER_PALE,.70)),'waterPrisonInternalFragment',index);fragment.userData.angle=angle;fragment.userData.radius=radius*(.18+(index%3)*.12);fragment.userData.speed=(index%2?1:-1)*(1.15+index*.09);fragment.position.set(Math.cos(angle)*fragment.userData.radius,(.22*Math.sin(angle*1.4))*size,Math.sin(angle)*fragment.userData.radius);group.add(remember(fragment));markers.fragments++;}
    if(renderMode==='style')for(let index=0;index<10;index++){const angle=index*Math.PI/5,droplet=tag(new THREE.Mesh(new THREE.SphereGeometry((.055+(index%3)*.015)*size,8,5),material(THREE,index%3?WATER:WATER_WHITE,.68)),'waterPrisonAttachedDroplet',index);droplet.userData.angle=angle;droplet.userData.radius=radius*(.88+(index%2)*.08);droplet.userData.speed=(index%2?1:-1)*(1.4+(index%4)*.12);droplet.position.set(Math.cos(angle)*droplet.userData.radius,(.34*Math.sin(angle*1.8))*size,Math.sin(angle)*droplet.userData.radius);group.add(remember(droplet));markers.droplets++;}
  }
  group.userData.visualMarkers=markers;group.renderOrder=8;scene.add(group);return{mesh:group,markers};
}
...
export function animateWaterPrisonVisual({mesh,phase='carrier',age=0,dt=0,now=0,size=1}={}){
  mesh?.children?.forEach((child,index)=>{const role=child.userData?.visualRole,base=child.userData?.baseScale||{x:1,y:1,z:1};
    ...
    else if(['waterPrisonShellPatch','waterPrisonInternalFragment','waterPrisonAttachedDroplet'].includes(role)){const angle=child.userData.angle+age*child.userData.speed,radius=child.userData.radius;child.position.x=Math.cos(angle)*radius;child.position.z=Math.sin(angle)*radius;child.position.y=(role==='waterPrisonInternalFragment'?.28:.36)*Math.sin(angle*(role==='waterPrisonInternalFragment'?1.7:1.3))*size;child.rotation.y=-angle;}
    else if(role==='waterPrisonAttachedShell'){const pulse=1+.025*Math.sin(now*6);child.scale.set(base.x*pulse,base.y*(2-pulse),base.z*pulse);}
  });
}
