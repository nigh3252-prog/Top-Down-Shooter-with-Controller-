...
export function createCombatEnemySystem({ THREE, worldRoot, dungeonScale = 6.5, materials = {}, directorOptions = {} }){
  ...
  const bandMaterials = {
    tooClose: new THREE.MeshBasicMaterial({ color:0xff6b55, transparent:true, opacity:.18, depthWrite:false }),
    hold: new THREE.MeshBasicMaterial({ color:0x8dd2ff, transparent:true, opacity:.13, depthWrite:false }),
    commit: new THREE.MeshBasicMaterial({ color:0xffd36a, transparent:true, opacity:.16, depthWrite:false })
  };
  ...
  function getCombatBand(e){
    ...
    const base = {
      ...
      brute:{ commitRange:2.35, holdRange:3.35, tooCloseRange:2.15, retreatRange:2.65, orbitAmount:.32 },
      chaser:{ commitRange:1.75, holdRange:2.35, tooCloseRange:1.45, retreatRange:1.85, orbitAmount:.62 }
  ...
  function orbit(e, p, dt, amount=.55, desiredRange=null){
    ...
    const dir = norm(tangent.x * .9 + away.x * radial, tangent.z * .9 + away.z * radial);
    steer(e, dir.x || toward.x, dir.z || toward.z, amount, dt); e.facing = norm(p.x - e.x, p.z - e.z);
  }
