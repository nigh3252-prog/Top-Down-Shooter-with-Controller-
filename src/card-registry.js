...
import { arcanaDownStanceId, arcanaElementStanceClass } from './arcana-stance-pairings.js';
import { getStanceClass } from './stance-compatibility.js';
...
for(const contribution of CONTRIBUTIONS){
  for(const sourceCard of contribution.cards){
    ...
    if(contribution.family==='arcana'){
      const stanceId=arcanaDownStanceId(card.arcanaId);
      if(!stanceId)throw new Error(`Arcana ${id} is missing a down stance pairing`);
      const expectedClass=arcanaElementStanceClass(card.element);
      if(expectedClass&&getStanceClass(stanceId)!==expectedClass)throw new Error(`Arcana ${id} must pair ${card.element} with a ${expectedClass} stance`);
    }
