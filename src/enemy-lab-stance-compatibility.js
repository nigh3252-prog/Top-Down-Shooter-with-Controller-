...
function makeChoice(document,{label,sub='',className='',onClick}){
  const button=document.createElement('button');button.className=`choice${className?` ${className}`:''}`;
...
function makeStatus(document,title,body,tone=''){
  const card=document.createElement('div');card.className='statusCard';card.dataset.stanceCompatibilityStatus='1';
...
function catchDescription(catchState){
  if(!catchState)return['GATE 4 · WAITING','Stance Catch runtime has not attached yet.',''];
  const multiplier=Number(catchState.windowMultiplier)||1;
  const duration=Number(catchState.windowDuration)||.72;
  const overdraw=Math.max(0,Number(catchState.trigger?.overdrawAmount)||0);
  if(catchState.phase==='open')return[
    overdraw>0?'GATE 4 · OVERDRAW':'GATE 4 · FINAL RESERVE',
    `${catchState.remaining.toFixed(2)} sec remain · window ×${multiplier} (${duration.toFixed(2)} sec)${overdraw>0?` · virtual cost +${overdraw.toFixed(0)}`:''}. Play a true stance card.`,
    'catch',
  ];
  if(catchState.phase==='success')return['GATE 4 · CLEAN STANCE CATCH','The stance transition succeeded and post-swing movement recovery was cleared.','catch'];
...
export function installEnemyLabStanceCompatibility({document=globalThis.document,runtime=null,sectionRegistry=null}={}){
  ...
  function render(force=false){
    ...
    }else root.appendChild(makeStatus(document,'OUTSIDE GATE 2 PILOT',`Gate 2 currently covers ${GATE2_STANCE_IDS.join(', ')} crossed with ${GATE2_WEAPON_IDS.join(', ')}. This combination still uses existing combat behavior.`));
