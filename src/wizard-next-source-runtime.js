...
export function installWizardNextSourceRuntime({THREE,scene,getPlayer,getEnemySystem,getMazeSegments=()=>[],advancePlayer=()=>false}={}){
  ...
  const state={effects:[],sizeMultiplier:initial.sizeMultiplier,elapsed:0,castSerial:0,lastCast:null,semanticSerial:0,semanticEvents:[],voltComboPress:0,voltComboRemaining:0,voltComboSerial:0,visualMode:'style'};
  ...
  function reset(){for(const effect of[...state.effects])remove(effect);state.elapsed=0;state.castSerial=0;state.lastCast=null;state.semanticSerial=0;state.semanticEvents.length=0;state.voltComboPress=0;state.voltComboRemaining=0;state.voltComboSerial=0;state.visualMode='style';}
  ...
  function emitVoltBurst(position,size,{kind,primary=null,press=0,comboSerial=0,visualMode='style'}={}){
    ...
    add({type:'voltDiscBurst',semanticKind:kind,visualMode,age:0,life:wall?.16:.30,mesh,direct:contact,harmless:wall,press,comboSerial,position:{...position},damage:kind==='terminal'?spec.missBurstDamage:0,targetIds});
  }
  function pressVoltDisc(detail={}){
    if(state.voltComboPress<=0||state.voltComboRemaining<=0||state.voltComboPress>=VOLT_DISC_COMBO.presses){state.voltComboPress=0;state.voltComboSerial++;semantic('volt-disc-combo-start',{arcanaId:'VOLT-DISC',comboSerial:state.voltComboSerial,total:VOLT_DISC_COMBO.presses,rollingTimeout:VOLT_DISC_COMBO.rollingTimeout});}
    state.voltComboPress++;state.voltComboRemaining=VOLT_DISC_COMBO.rollingTimeout;const press=state.voltComboPress,comboSerial=state.voltComboSerial;
    emitVoltDisc(press,comboSerial);
    if(press>=VOLT_DISC_COMBO.presses){semantic('volt-disc-combo-complete',{arcanaId:'VOLT-DISC',comboSerial,press,total:VOLT_DISC_COMBO.presses,deckSequence:detail?.sequence||null});state.voltComboPress=0;state.voltComboRemaining=0;}
    return true;
  }
  function startCombo(type){const life=type==='earthCombo'?1.05:type==='vineCombo'?.92:type==='stoneCombo'?.96:type==='sparkCombo'?.72:.76;add({type,age:0,nextBeat:0,life});}
  ...
  return{state,cast,snapshot,reset,update(dt,now=0){
    ...
    if(state.voltComboPress>0&&state.voltComboRemaining>0){state.voltComboRemaining=Math.max(0,state.voltComboRemaining-frame);if(state.voltComboRemaining<=0){semantic('volt-disc-combo-expired',{arcanaId:'VOLT-DISC',comboSerial:state.voltComboSerial,press:state.voltComboPress,total:VOLT_DISC_COMBO.presses});state.voltComboPress=0;}}
