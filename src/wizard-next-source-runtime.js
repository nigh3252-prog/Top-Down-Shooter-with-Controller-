...
export function installWizardNextSourceRuntime({THREE,scene,getPlayer,getEnemySystem,getMazeSegments=()=>[],advancePlayer=()=>false}={}){
  ...
  function startCombo(type){add({type,age:0,nextBeat:0,life:type==='earthCombo'?1.05:(type==='vineCombo'?.92:(type==='stoneCombo'?.96:.72))});}
  ...
  const onPlay=event=>{const id=event?.detail?.card?.arcanaId;if(id==='EARTH-KNUCKLES')startCombo('earthCombo');else if(id==='BLADED-VINE')startCombo('vineCombo');else if(id==='STONE-SHOT')startCombo('stoneCombo');else if(id==='SPARK-CONTACT')startCombo('sparkCombo');};
  ...
  return{state,reset,update(dt,now=0){const frame=Math.max(0,Number(dt)||0),time=Number(now)||0;for(const effect of[...state.effects]){if(effect.type==='earthCombo')updateCombo(effect,frame,EARTH_KNUCKLES_BEATS,emitEarthPunch);else if(effect.type==='vineCombo')updateCombo(effect,frame,BLADED_VINE_BEATS,emitVine);else if(effect.type==='stoneCombo')updateCombo(effect,frame,STONE_SHOT_BEATS,emitStone);else if(effect.type==='sparkCombo')updateCombo(effect,frame,SPARK_CONTACT_BEATS,emitSpark);else if(effect.type==='earthPunch')updateEarthPunch(effect,frame);else if(effect.type==='vineStrike')updateVine(effect,frame,time);else if(effect.type==='stoneProjectile')updateStone(effect,frame,time);else if(effect.type==='sparkStrike')updateSpark(effect,frame);else if(effect.type==='impactOnly')updateImpact(effect,frame);}},dispose(){window.removeEventListener('wizard-arcana:play',onPlay);window.removeEventListener(ARCANA_TWEAKS_EVENT,onTweaks);reset();}};
}
