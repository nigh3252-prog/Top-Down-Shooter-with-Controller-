...
The Stone core no longer carries a duplicate inline weapon table. The official Stone Weapon Lab wrapper injects weapon data from `weapons.js` into the iframe before the core module runs. Future cleanup passes should pull out the next seams one at a time, such as attacks, input, enemies, UI, and audio. The short-term goal is one official Stone Weapon Lab entry point with donor/reference pages kept only while they remain useful.
