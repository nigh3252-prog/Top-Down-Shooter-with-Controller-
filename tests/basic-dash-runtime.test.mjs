...
function makeScenario({ moveX=0, moveZ=0 }={}){
  ...
  Object.defineProperty(globalThis, 'navigator', {
    configurable:true,
    value:{ getGamepads(){ return [gamepad]; } },
  });
...
backward.runtime.update(.016);
assert.equal(backward.actorPos.x, 0, 'lead-in should not move the collision body');
assert.equal(backward.actorPos.y, 0, 'lead-in should not move the collision body');
assert.ok(backward.activeModel.position.y < 3.05, 'lead-in should compress the Warden slightly');
assert.ok(backward.activeModel.rotation.x > 0, 'backward dash anticipation should lean the Warden forward');
assert.equal(backward.facing, 0, 'idle backward dash should preserve the original facing');

let backwardEnd=null;
for(let frame=0; frame<40; frame++){
  backward.runtime.update(.016);
  if(!backward.runtime.state.active && backward.runtime.state.postActive && backwardEnd===null){
    backwardEnd={ x:backward.actorPos.x, z:backward.actorPos.y };
  }
}
...
sideways.runtime.update(.016);
assert.ok(sideways.activeModel.rotation.z > 0, 'right dash anticipation should lean the Warden left before launch');
let sidewaysEnd=null;
for(let frame=0; frame<40; frame++){
  sideways.runtime.update(.016);
  if(!sideways.runtime.state.active && sideways.runtime.state.postActive && sidewaysEnd===null){
    sidewaysEnd={ x:sideways.actorPos.x, z:sideways.actorPos.y };
  }
}
