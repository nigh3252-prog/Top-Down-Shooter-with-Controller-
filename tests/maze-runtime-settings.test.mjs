...
import {
  ...
  configuredHexSize,
  configuredRoomSize,
  getArenaMazeSettings,
  getLabChamberSettings,
...
assert.deepEqual(getArenaMazeSettings().cellSize, MAZE_CELL_SIZE_OPTIONS.find(option => option.id === 'compact'));
assert.deepEqual(getLabChamberSettings().cellSize, MAZE_CELL_SIZE_OPTIONS.find(option => option.id === 'large'));
