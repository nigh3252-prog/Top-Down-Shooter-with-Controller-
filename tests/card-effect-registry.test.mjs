...
assert.equal(CARD_DEFINITIONS.length,89,'the registry inventory must include every current card');
...
assert.equal(listCards({family:'arcana'}).length,56);
const effectCards=[...listCards({family:'non-stance'}),...listCards({family:'arcana'})];
assert.equal(effectCards.length,58,'the canonical non-stance inventory is Pilebunker, Blood Slash, and 56 Arcana');
assert.ok(effectCards.every(card=>card.effectId),'every canonical non-stance card must own an effectId');
assert.equal(ARCANA_EFFECT_DEFINITIONS.length,56);
...
assert.equal(EFFECT_DEFINITIONS.length,59,'Bing Bong plus 58 non-stance effects must be registered');
...
assert.equal(productionDispatcher.list().length,59);
