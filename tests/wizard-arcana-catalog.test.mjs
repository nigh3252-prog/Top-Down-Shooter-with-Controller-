...
const canonicalIds=[
  ...
  ...nextTwentyIds,
  'HOMING-FLARES','DRAGON-ARC','WHIRLING-TORNADO','WATER-PRISON',
];
assert.equal(WIZARD_ARCANA_CATALOG.length,36);
assert.equal(new Set(WIZARD_ARCANA_CATALOG.map(card=>card.id)).size,36);
...
assert.deepEqual(WIZARD_ARCANA_CATALOG.map(wizardArcanaSourceOrder),[
  ...Array.from({length:32},(_,index)=>index+1),42,44,59,68,
]);

const originalCards=[...WIZARD_ARCANA_CARDS,...WIZARD_AIR_BASIC_CARDS,...WIZARD_NEXT_SOURCE_CARDS];
