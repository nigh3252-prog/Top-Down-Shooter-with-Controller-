...
import { createRedDashBurst } from '../src/magic-brush-presets.js';

const backward=createRedDashBurst({position:{x:2,z:3},dashDirection:{x:0,z:-1}});
...
const sideways=createRedDashBurst({position:{x:0,z:0},dashDirection:{x:1,z:0}});
...
assert.equal(field.state.active,true,'a dash burst should wake the field');
const initial=field.materialCentroid();
assert.ok(initial.z>backward.origin.z,'burst material should be painted in the thrust direction');
