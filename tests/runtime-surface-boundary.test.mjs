...
assert.match(runtime,/pad\.pressed\.dpadUp\)cycleWeapon\(-1\)/);
assert.match(runtime,/pad\.pressed\.dpadDown\)cycleWeapon\(1\)/);
assert.doesNotMatch(runtime,/installRoadieRun|roadie-run\.js/);
...
assert.match(runtime,/createStanceGate4Runtime/);
assert.match(runtime,/installStanceGate4RingSize/);
assert.match(runtime,/createStanceGate5Runtime/);
assert.match(runtime,/readPlayStationBackboneInput/);
assert.match(runtime,/resolvePlayStationBackboneActions/);
assert.match(runtime,/if\(!ABILITY_CAPTURE_MODE\)[\s\S]*installStanceGate2Runtime\(\{arenaHandle:runtimeHandle\}\)[\s\S]*installStanceGate3Runtime\(\{arenaHandle:runtimeHandle,gate2Runtime:stanceGate2Runtime\}\)[\s\S]*createStanceGate4Runtime\(\{arenaHandle:runtimeHandle,gate3Runtime:stanceGate3Runtime/);
assert.match(runtime,/stanceGate5Runtime\?\.destroy\?\.\(\)[\s\S]*stanceGate4RingSize\?\.destroy\?\.\(\)[\s\S]*stanceGate4Runtime\?\.destroy\?\.\(\)[\s\S]*stanceGate3Runtime\?\.destroy\?\.\(\)[\s\S]*stanceGate2Runtime\?\.destroy\?\.\(\)/);
