...
const firstGeneratedJet=buildPerforatingJetSchedule({jetBeat:1});
const secondGeneratedJet=buildPerforatingJetSchedule({jetBeat:2});
assert.equal(baseSchedule.length,9);
assert.equal(enhancedSchedule.length,12);
assert.equal(firstGeneratedJet.length,2,'a generated first Jet card emits only the source two-shot volley');
assert.equal(secondGeneratedJet.length,3,'a generated second Jet card emits only the source three-shot volley');
assert.ok(firstGeneratedJet.every(event=>event.beat===1));
assert.ok(secondGeneratedJet.every(event=>event.beat===2));
