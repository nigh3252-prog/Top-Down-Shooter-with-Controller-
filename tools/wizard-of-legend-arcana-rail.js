(()=>{
  ...
  const VFX_LAB_IMPLEMENTED=['flame-breath','searing-crown','ignition-drive','engulfing-fissure','dragon-blast','shearing-chain','tectonic-drill','rock-solid-tomahawk','aqua-vortex','aqua-breaker'];
  const CATALOG_MIGRATIONS={2:{'dragon-arc':{analysis:true,implementation:true}},3:Object.fromEntries(NEXT_TWENTY_IMPLEMENTED.map(id=>[id,{analysis:true,implementation:true}])),4:Object.fromEntries(ARCHETYPE_SAMPLER_IMPLEMENTED.map(id=>[id,{analysis:true,implementation:true}])),6:Object.fromEntries(VFX_LAB_IMPLEMENTED.map(id=>[id,{analysis:true,implementation:true}]))};
  ...
  function searchBlob(entry){return JSON.stringify([entry.name,entry.summary,entry.recipe,entry.units,entry.analysisMarkdown,entry.supplementMarkdown,entry.onlineReferenceMarkdown]).toLowerCase();}
  ...
  function renderResearch(kind){
    ...
    const sources='<div class="rail-notice"><strong>Unofficial educational reference.</strong> Wizard of Legend and its assets belong to their respective owners. This page analyzes publicly demonstrated behavior for prototype study; it does not contain original game code and does not claim ownership of the footage.</div><p>The local 480p reference video is 69 MiB, below GitHub\'s 100 MiB per-file ceiling and within the current Pages site budget. See <a href="https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-large-files-on-github" target="_blank" rel="noreferrer">GitHub repository file limits</a> and <a href="https://docs.github.com/en/pages/getting-started-with-github-pages/github-pages-limits" target="_blank" rel="noreferrer">GitHub Pages limits</a>. The source video is 22:09 at 60 FPS. The full showcase and individual clip windows are available above.</p><p><a href="https://wol.spread.name/" target="_blank" rel="noreferrer">Wizard of Legend community Arcana catalog ↗</a> · <a href="https://wizardoflegend.fandom.com/wiki/Arcana" target="_blank" rel="noreferrer">Wiki Arcana index ↗</a></p><p>Generated from the archived project research on 2026-07-27T04:40:53.046Z.</p>';
  ...
  async function copyBrief(entry){
    ...
    const online=entry.onlineReference;
    const text=[`${entry.name} — Wizard of Legend source reference`,`${location.href.split('#')[0]}#arcana-${entry.id}`,`Full showcase clip: ${formatTime(clip.start)}–${formatTime(clip.end)}`,segmentSummary(entry),entry.referenceWindow?`Base reference lock: ${formatTime(entry.referenceWindow.start)}–${formatTime(entry.referenceWindow.end)}`:'',`Lineage: ${lineageLabels[entry.lineage]??entry.lineage}`,`Status: ${statusLabels[entry.status]??entry.status}`,`Stages: analysis ${stage.analysis?'complete':'pending'}; implementation ${stage.implementation?'complete':'pending'}; comparison ${stage.comparison?'passed':'pending'}`,'',entry.summary,online?.documentedBehavior?`Documented online behavior: ${online.documentedBehavior}`:'',online?.enhanced?`Documented online enhanced behavior: ${online.enhanced}`:'',online?.sourceUrl?`Online source: ${online.sourceUrl}`:'',plainForCopy(entry.recipe),'',plainForCopy(entry.acceptance),'',plainForCopy(entry.issues)].filter(Boolean).join('\n');
