/* =============================================================================
   Star Bolt (#108) and Shock Nova (#102) — Lightning Arcana, rebuilt in three.js.

   Every timing, radius, colour and beat below was read off the Arcana showcase
   encode shipped with the archive (854x480 @ 30fps):
     Shock Nova  13:28–13:41  (base 808–814 s, charged 814–821 s)
     Star Bolt   14:29–14:35  (869–875 s)
   One world unit == one floor tile == ~48 px in that encode.
   ========================================================================== */
...
scene.fog = new T.Fog(0x0a0c10, 20, 40);

/* The showcase camera sits high and slightly back — about 60° off the floor. */
const camera = new T.PerspectiveCamera(27, innerWidth / innerHeight, 0.1, 200);
const CAM_OFFSET = new T.Vector3(0, 20.6, 9.2);
...
/* ================================================================ lights == */
scene.add(new T.HemisphereLight(0xa8b4c6, 0x3a3d44, 2.05));
const key = new T.DirectionalLight(0xfff2d8, 0.85);
key.position.set(-6, 14, 7);
scene.add(key);

/* One coloured light drives the floor wash — gold while a nova burns, cyan on
   every shock discharge, exactly the way the source frames blow out. */
const spellLight = new T.PointLight(0xffe6a0, 0, 26, 2);
...
function makeFloorTexture() {
  ...
  const CELL = S / 8;                       // 8 flagstones per texture tile
  for (let y = 0; y < 8; y++) {
    for (let x = 0; x < 8; x++) {
      const px = x * CELL, py = y * CELL;
      const v = 0.86 + Math.random() * 0.28;
      const base = Math.round(92 * v);
      g.fillStyle = `rgb(${base + 4},${base + 6},${base + 9})`;
      const inset = 3 + Math.random() * 2;
      roundRect(g, px + inset, py + inset, CELL - inset * 2, CELL - inset * 2, 7);
      g.fill();
      // top-left bevel, bottom-right shade — the flagstones read as raised
      g.strokeStyle = `rgba(255,255,255,${0.05 + Math.random() * 0.04})`;
      g.lineWidth = 2;
      g.beginPath();
      g.moveTo(px + inset + 3, py + CELL - inset - 4);
      g.lineTo(px + inset + 3, py + inset + 4);
      g.lineTo(px + CELL - inset - 4, py + inset + 3);
      g.stroke();
      g.strokeStyle = 'rgba(0,0,0,.22)';
      g.beginPath();
      g.moveTo(px + CELL - inset - 3, py + inset + 5);
      g.lineTo(px + CELL - inset - 3, py + CELL - inset - 3);
      g.lineTo(px + inset + 5, py + CELL - inset - 3);
      g.stroke();
      // speckle
      for (let i = 0; i < 16; i++) {
        g.fillStyle = `rgba(${Math.random() < .5 ? 0 : 255},${Math.random() < .5 ? 0 : 255},255,${0.03 + Math.random() * 0.05})`;
        const s = 1 + Math.random() * 2.2;
        g.fillRect(px + inset + Math.random() * (CELL - inset * 2 - s), py + inset + Math.random() * (CELL - inset * 2 - s), s, s);
      }
      // the occasional hairline crack
      if (Math.random() < 0.18) {
        g.strokeStyle = 'rgba(0,0,0,.3)'; g.lineWidth = 1.4;
        g.beginPath();
        let cx = px + inset + Math.random() * (CELL - inset * 2);
        let cy = py + inset + 4;
        g.moveTo(cx, cy);
        for (let i = 0; i < 4; i++) { cx += (Math.random() - .5) * 14; cy += (CELL - 2 * inset) / 4; g.lineTo(cx, cy); }
        g.stroke();
      }
    }
  }
...
const floor = new T.Mesh(
  ...
  new T.MeshLambertMaterial({ map: makeFloorTexture() })
);
...
addEventListener('resize', () => { sparks.mat.uniforms.uScale.value = innerHeight * 0.6; });
...
function makeWizard() {
  const g = new T.Group();
  const cloak = new T.Mesh(
    new T.ConeGeometry(0.30, 0.62, 10, 1, true),
    new T.MeshLambertMaterial({ color: 0xd8cdb2, side: T.DoubleSide })
  );
  cloak.position.y = 0.31; g.add(cloak);
  const tunic = new T.Mesh(
    new T.CylinderGeometry(0.17, 0.20, 0.36, 8),
    new T.MeshLambertMaterial({ color: 0x6d2f2c })
  );
  tunic.position.y = 0.30; g.add(tunic);
  const belt = new T.Mesh(
    new T.CylinderGeometry(0.205, 0.205, 0.05, 10),
    new T.MeshBasicMaterial({ color: 0xe0a52c })
  );
  belt.position.y = 0.29; g.add(belt);
  const legs = new T.Mesh(
    new T.CylinderGeometry(0.15, 0.13, 0.22, 8),
    new T.MeshLambertMaterial({ color: 0x181820 })
  );
  legs.position.y = 0.11; g.add(legs);
  const head = new T.Mesh(
    new T.SphereGeometry(0.155, 12, 10),
    new T.MeshLambertMaterial({ color: 0xf0e6d2 })
  );
  head.position.y = 0.70; g.add(head);
  const visor = new T.Mesh(
    new T.BoxGeometry(0.26, 0.105, 0.06),
    new T.MeshBasicMaterial({ color: 0x14141a })
  );
  visor.position.set(0, 0.70, 0.135); g.add(visor);
  const hood = new T.Mesh(
    new T.ConeGeometry(0.20, 0.30, 10),
    new T.MeshLambertMaterial({ color: 0xcfc3a6 })
  );
  hood.position.y = 0.85; hood.rotation.x = -0.18; g.add(hood);

  const shadow = makeShadow(0.62);
  g.add(shadow);
  g.userData = { head, cloak, hood, shadow };
  return g;
}
function makeShadow(size) {
  const s = new T.Mesh(
    new T.PlaneGeometry(size, size * 0.62),
    new T.MeshBasicMaterial({
      map: radialSprite([[0, 'rgba(0,0,0,.55)'], [0.55, 'rgba(0,0,0,.28)'], [1, 'rgba(0,0,0,0)']], 64),
      transparent: true, depthWrite: false,
    })
  );
  s.rotation.x = -Math.PI / 2; s.position.y = 0.012; s.renderOrder = 2;
  return s;
}

function makeDummy() {
  const g = new T.Group();
  const woodMat = new T.MeshLambertMaterial({ color: 0xc08c4c });
  const post = new T.Mesh(new T.CylinderGeometry(0.115, 0.13, 0.62, 8), woodMat);
  post.position.y = 0.42; g.add(post);
  const head = new T.Mesh(new T.CylinderGeometry(0.115, 0.115, 0.20, 8), woodMat);
  head.position.y = 0.80; g.add(head);
  const arms = new T.Mesh(new T.BoxGeometry(0.72, 0.11, 0.11), woodMat);
  arms.position.y = 0.61; g.add(arms);
  const base = new T.Mesh(
    new T.CylinderGeometry(0.26, 0.29, 0.14, 12),
    new T.MeshLambertMaterial({ color: 0x8b8f96 })
  );
  base.position.y = 0.07; g.add(base);
  g.add(makeShadow(0.7));

  const aura = makeDecal(TEX.glow, PAL.cyanHot, 0);
  aura.scale.set(1.35, 1.35, 1);
  g.userData = {
    mats: [woodMat],
    baseColor: new T.Color(0xc08c4c),
    parts: [post, head, arms],
    aura,
    /* state */
...
scene.add(player);
const playerState = { pos: new T.Vector3(0, 0, 1.6), facing: 0, crouch: 0, throwT: 0 };

const dummies = [];
function layoutDummies() {
  dummies.forEach(d => scene.remove(d));
  dummies.length = 0;
  /* The showcase parks a loose cluster of five practice dummies to the caster's
     right, close enough that the base nova rim just reaches the far ones. */
  const spots = [
    [ 1.20, -1.05], [ 1.95, -0.15], [ 1.05,  0.70],
    [ 1.85,  1.15], [ 2.95,  0.45], [ 0.15,  1.55],
  ];
  spots.forEach(([x, z]) => {
    const d = makeDummy();
    d.position.set(x, 0, z);
    d.userData.home.set(x, 0, z);
    scene.add(d);
    dummies.push(d);
  });
}
...
/* ============================================================ damage HUD == */
const dmgPool = [];
function popNumber(worldPos, text, kind) {
  let el = dmgPool.pop();
  ...
  const item = {
    el, pos: worldPos.clone(), t: 0,
    life: kind === 'burst' ? 1.15 : 0.85,
    rise: kind === 'burst' ? 0.9 : 1.35,
...
function updateNumbers(dt) {
  for (let i = numbers.length - 1; i >= 0; i--) {
    ...
    const k = n.t / n.life;
    if (k >= 1) {
      n.el.remove(); dmgPool.push(n.el); numbers.splice(i, 1); continue;
    }
    projV.copy(n.pos);
    projV.y += 0.9 + n.rise * n.t;
    projV.x += n.drift * n.t;
    projV.project(camera);
    n.el.style.left = ((projV.x * 0.5 + 0.5) * innerWidth) + 'px';
    n.el.style.top = ((-projV.y * 0.5 + 0.5) * innerHeight) + 'px';
    n.el.style.opacity = k > 0.62 ? String(1 - (k - 0.62) / 0.38) : '1';
    const pop = k < 0.14 ? 0.6 + (k / 0.14) * 0.55 : 1.0;
...
function shake(amount, dur) { shakeAmt = Math.max(shakeAmt, amount); shakeT = shakeDur = dur; }
let lightHold = 0;
function lightPulse(color, intensity, dur) {
  spellLight.color.copy(color);
  spellLight.intensity = Math.max(spellLight.intensity, intensity);
  lightHold = Math.max(lightHold, dur);
}
...
addEventListener('pointermove', e => {
  ...
  if (raycaster.ray.intersectPlane(groundPlane, hitPoint)) {
    aim.copy(hitPoint).sub(playerState.pos).setY(0);
    if (aim.lengthSq() > 1e-6) aim.normalize();
  }
});
renderer.domElement.addEventListener('pointerdown', e => {
  if (showcase.running) stopShowcase();
  if (e.button === 0) throwStar();
  if (e.button === 2) beginNova();
});
addEventListener('pointerup', e => { if (e.button === 2) releaseNova(); });
addEventListener('keydown', e => {
  const k = e.key.toLowerCase();
  if (k === ' ') {
    e.preventDefault();
    if (!keys.has(' ')) { if (showcase.running) stopShowcase(); beginNova(); }
  }
  keys.add(k);
  if (k === 'e') setEnhanced(!enhanced);
  if (k === 'r') { layoutDummies(); }
  if (k === 'h') togglePanel();
});
...
function throwStar() {
  if (starCooldown > 0 || novaCharge) return;
  starCooldown = TUNE.star.cadence;
  playerState.throwT = 0.26;
  playerState.facing = Math.atan2(aim.x, aim.z);
  const dir = aim.clone();
  /* the shuriken leaves the hand a beat after the cloak sweep starts */
  setTimeout(() => {
    const origin = playerState.pos.clone().addScaledVector(dir, 0.42);
    effects.push(new StarBolt(origin, dir));
    burstSparks(origin, 12, 2.6, PAL.goldHot, 0.28, 0.8, 1.4);
    lightPulse(PAL.gold, 4, 0.1);
  }, TUNE.star.windup * 1000);
}
function beginNova() {
  if (novaCharge) return;
  novaCharge = new NovaCharge();
  effects.push(novaCharge);
}
function releaseNova() {
  if (!novaCharge) return;
  const charge = novaCharge;
  novaCharge = null;
  const full = charge.progress >= 0.999;
  const i = effects.indexOf(charge);
  if (i >= 0) { effects.splice(i, 1); charge.dispose(); }
  /* the source only ever fires this at full charge — an early release still
     goes off, just at the base radius and with a shorter burn */
  const nova = new ShockNova(playerState.pos, enhanced && full);
  if (!full) { nova.R *= 0.55 + 0.45 * charge.progress; nova.life *= 0.7; }
  effects.push(nova);
}
function setEnhanced(v) {
  if (enhanced === v) return;
  enhanced = v;
  btnEnh.textContent = 'Enhanced: ' + (v ? 'on' : 'off');
  btnEnh.classList.toggle('on', v);
  arcCharged.classList.toggle('on', v);
  if (v) effects.push(new EmpowerFlare(playerState.pos));
}

/* ============================================================== showcase ==
   A straight replay of the source beats: the nova demonstration, then the
   star demonstration, then the same two again with the card charged. */
const showcase = { running: false, script: [], t: 0, i: 0 };
function playShowcase() {
  stopShowcase();
  layoutDummies();
  playerState.pos.set(0, 0, 1.6);
  aim.set(1, 0, -0.15).normalize();
  setEnhanced(false);
  const S = [];
  let t = 0;
  const at = (dt, fn) => { t += dt; S.push({ t, fn }); };

  at(0.6, () => { setArcana('SHOCK NOVA'); beginNova(); });
  at(TUNE.nova.chargeTime + 0.02, () => releaseNova());
  at(4.6, () => { setArcana('STAR BOLT'); });
  for (let i = 0; i < 5; i++) at(i === 0 ? 0.4 : TUNE.star.cadence, () => throwStar());
  at(4.4, () => { setArcana('SHOCK NOVA'); setEnhanced(true); });
  at(1.5, () => beginNova());
  at(TUNE.nova.chargeTimeEnh + 0.02, () => releaseNova());
  at(5.4, () => { setArcana('STAR BOLT'); });
  for (let i = 0; i < 4; i++) at(i === 0 ? 0.4 : TUNE.star.cadence, () => throwStar());
  at(5.6, () => { stopShowcase(); });

  showcase.script = S; showcase.t = 0; showcase.i = 0; showcase.running = true;
  btnShowcase.classList.add('on');
  btnShowcase.textContent = '■ Stop showcase';
}
function stopShowcase() {
  showcase.running = false;
  btnShowcase.classList.remove('on');
  btnShowcase.textContent = '▶ Play showcase';
}
function setArcana(name) { arcName.textContent = name; }

/* ================================================================== HUD === */
const btnShowcase = document.getElementById('btnShowcase');
const btnEnh = document.getElementById('btnEnh');
const btnNova = document.getElementById('btnNova');
const btnStar = document.getElementById('btnStar');
const arcName = document.getElementById('arcName');
const arcCharged = document.getElementById('arcCharged');
const meterFill = document.getElementById('meterFill');
const uiPanel = document.getElementById('ui');
const collapse = document.getElementById('collapse');

btnShowcase.onclick = () => showcase.running ? stopShowcase() : playShowcase();
btnEnh.onclick = () => setEnhanced(!enhanced);
btnNova.onclick = () => {
  if (showcase.running) stopShowcase();
  setArcana('SHOCK NOVA');
  beginNova();
  const wait = (enhanced ? TUNE.nova.chargeTimeEnh : TUNE.nova.chargeTime) + 0.03;
  setTimeout(releaseNova, wait * 1000);
};
btnStar.onclick = () => { if (showcase.running) stopShowcase(); setArcana('STAR BOLT'); throwStar(); };
function togglePanel() {
  const hidden = uiPanel.style.display === 'none';
  uiPanel.style.display = hidden ? '' : 'none';
  collapse.style.display = hidden ? 'none' : 'block';
}
collapse.onclick = togglePanel;
...
function updatePlayer(dt) {
  ...
  if (keys.has('a')) mx -= 1;
  if (keys.has('d')) mx += 1;
  const moving = (mx || mz) && !novaCharge;
  if (moving) {
    const l = Math.hypot(mx, mz);
    const sp = 3.4 * dt;
    playerState.pos.x = T.MathUtils.clamp(playerState.pos.x + (mx / l) * sp, -14, 14);
    playerState.pos.z = T.MathUtils.clamp(playerState.pos.z + (mz / l) * sp, -14, 14);
  }
  /* the caster drops into a crouch while a nova is charging */
  const wantCrouch = novaCharge ? 1 : 0;
  playerState.crouch += (wantCrouch - playerState.crouch) * Math.min(1, dt * 12);
  playerState.throwT = Math.max(0, playerState.throwT - dt);

  player.position.copy(playerState.pos);
  const face = Math.atan2(aim.x, aim.z);
  playerState.facing += T.MathUtils.clamp(
    ((face - playerState.facing + Math.PI * 3) % (Math.PI * 2)) - Math.PI, -12 * dt, 12 * dt);
  player.rotation.y = playerState.facing;
  player.scale.set(1 + playerState.crouch * 0.10, 1 - playerState.crouch * 0.16, 1 + playerState.crouch * 0.10);

  /* cloak sweep on the throw */
  const th = playerState.throwT / 0.26;
  const ud = player.userData;
  ud.cloak.rotation.x = -0.35 * Math.sin(th * Math.PI) + playerState.crouch * 0.12;
  ud.cloak.scale.z = 1 + 0.5 * Math.sin(th * Math.PI);
  ud.head.position.y = 0.70 - playerState.crouch * 0.04;
  ud.shadow.position.y = 0.012;
}
...
function updateDummies(dt) {
  for (const d of dummies) {
    ...
    /* white flash on contact, cyan while a shock stack is riding */
    const flashK = Math.max(0, u.flash / 0.2);
    const shocked = u.shock > 0;
    const mat = u.mats[0];
    if (flashK > 0) {
      mat.color.copy(PAL.white);
    } else if (shocked) {
      const pulse = 0.5 + 0.5 * Math.sin(performance.now() * 0.019);
      mat.color.copy(u.baseColor).lerp(PAL.cyan, 0.72).lerp(PAL.cyanHot, 0.18 + 0.34 * pulse);
    } else {
      mat.color.copy(u.baseColor);
    }

    u.aura.position.set(d.position.x, 0.03, d.position.z);
    u.aura.visible = shocked;
    if (shocked) {
      u.aura.material.opacity = 0.30 + 0.16 * Math.sin(performance.now() * 0.021 + d.position.x);
      u.arcT -= dt;
      if (u.arcT <= 0) {
        u.arcT = 0.05 + Math.random() * 0.09;
        tmp.set(d.position.x + (Math.random() - .5) * 0.5, 0.15 + Math.random() * 0.8, d.position.z + (Math.random() - .5) * 0.5);
        tmp2.set((Math.random() - .5) * 1.2, 0.6 + Math.random(), (Math.random() - .5) * 1.2);
        sparks.emit(tmp, tmp2, PAL.cyanHot, 0.2, 0.5, 2.0);
      }
      u.shockT -= dt;
      if (u.shockT <= 0) dischargeShock(d);
    }

    /* stagger + hop reaction */
    const hop = Math.max(0, u.hop / 0.28);
    d.position.y = Math.sin(hop * Math.PI) * 0.16;
    const lean = u.stun > 0 ? Math.sin(performance.now() * 0.03) * 0.10 : 0;
    d.rotation.z = lean;
    d.position.x += (u.home.x - d.position.x) * Math.min(1, dt * 6);
    d.position.z += (u.home.z - d.position.z) * Math.min(1, dt * 6);
  }
}
...
function updateShowcase(dt) {
  ...
  showcase.t += dt;
  while (showcase.i < showcase.script.length && showcase.t >= showcase.script[showcase.i].t) {
    showcase.script[showcase.i].fn();
    showcase.i++;
  }
}
...
function tick() {
  ...
  if (starCooldown > 0) starCooldown -= dt;
  updateShowcase(dt);
  updatePlayer(dt);
  updateEffects(dt);
  updateDummies(dt);
  ...
  /* light decay */
  if (lightHold > 0) lightHold -= dt;
  else spellLight.intensity *= Math.exp(-9 * dt);
  if (spellLight.intensity < 0.02) spellLight.intensity = 0;

  meterFill.style.width = novaCharge ? (novaCharge.progress * 100).toFixed(1) + '%' : '0%';

  /* camera follow + hit shake */
  camTarget.copy(playerState.pos).add(CAM_OFFSET);
  camera.position.lerp(camTarget, Math.min(1, dt * 4));
  if (shakeT > 0) {
    shakeT -= dt;
    const s = shakeAmt * (shakeT / shakeDur);
  ...
  camera.lookAt(playerState.pos.x, 0.45, playerState.pos.z);

  renderer.render(scene, camera);
}

/* A small scripted hook — handy for driving the page from the console or an
   automated capture. */
window.WOL = {
  throwStar, beginNova, releaseNova, setEnhanced, playShowcase, stopShowcase,
  setArcana, layoutDummies, TUNE, effects,
  get player() { return playerState; },
  aimAt(x, z) { aim.set(x - playerState.pos.x, 0, z - playerState.pos.z).normalize(); },
};

setArcana('SHOCK NOVA');
tick();
setTimeout(playShowcase, 700);
