...
The stable internal class IDs remain Light / Medium / Heavy, while player-facing UI calls them **Speed / Balanced / Power**. Stance-card art and the three-card room reward selector use compact **type / defense** badges: **S / D**, **B / P**, or **P / B**. Type is Speed (S), Balanced (B), or Power (P); defense is Dodge (D), Parry (P), or Block (B). `normalizeStanceClass()` accepts either vocabulary.
