...
- `tools/wizard-of-legend-air-arcana-demo.html` — standalone, phone-first three.js demo of two Air Arcana reconstructed from the showcase video: Storm Draft (base cast, 2 charges, wall-slam bonus, and the 4×7 charged signature) and Whirling Wind Agent (2 summoned agents, piercing charges, duration bars). Self-contained — three.js is inlined, so the file runs offline from disk. It does not import or modify the game runtime. Rebuild it with `node scripts/build-wol-air-arcana-demo.mjs` from `scripts/wol-air-arcana-template.html`, `scripts/wol-air-arcana-demo.js`, and `scripts/vendor/three.min.js`.
