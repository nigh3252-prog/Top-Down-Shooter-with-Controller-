...
- `tools/wizard-of-legend-arcana-demo.html` — a standalone, phone-first three.js reconstruction of two Arcana, **Rock N' Roll** (entry 92, showcase window 12:05–12:17) and **Earth Stomp Agent** (entry 97, 12:41–12:53), built frame-by-frame from `media/wizard-of-legend/wizard-of-legend-arcana-showcase-480p.mp4`. Everything — three.js included — is inlined, so the single file runs offline from a download with no server. It does not import or modify the game runtime. Sources live in `tools/arcana-demo/`; rebuild with `node scripts/build-arcana-demo.mjs`.
