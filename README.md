...
- `tools/wol-arcana-phone-demo.html` — standalone, touch-first three.js demo of two Arcana rebuilt straight from the showcase encode and the source notes: **Blazing Lariat** (37, video 244–255s) and **Explosive Charge** (39, video 259–265s). One self-contained file with three.js inlined, so it runs offline from `file://` with no network. It ships the exact source recipes, the authored constant tables, and a live acceptance checklist that ticks off each note's acceptance test as you demonstrate it in play. Edit `tools/wol-arcana-phone-demo.src.html`, then rebuild with `node scripts/build-wol-arcana-phone-demo.mjs` (needs `npm install --no-save three@0.165.0`). It does not import or modify the game runtime.
