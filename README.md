...
- `index.html` — launcher.
- `tools/wizard-of-legend-arcana-checklist.html` — standalone, phone-friendly source-analysis checklist covering all 149 Arcana in the showcase video, with bounded clips, separate charged demonstrations, persistent progress, and explicit source-first versus inventory-only versus legacy-replacement lineage. It does not import or modify the game runtime. Rebuild it with `node scripts/build-wol-checklist.mjs`; fill in missing clip posters with `node scripts/generate-wol-posters.mjs --missing`.
- `tools/wizard-arcana-phone-demo.html` — a standalone, phone-first three.js demo of two Arcana rebuilt from the showcase clip: **Bubble Barrage** (Water/Standard, clip 1091–1097 s) and **Blurring Falconry** (Air/Signature, clip 522–540 s, charged cast at 533.3 s). Everything is inlined — three.js r160, all textures generated on a canvas at boot — so the single file runs offline with no network access. Touch joystick on the left, tap-or-drag-to-aim Arcana buttons on the right, plus WASD/mouse on desktop. Includes the enhanced rewrites, the signature gauge, and the clip's presentation beats (element-tinted name card, white per-hit numbers, red running total on dummy reset). It does not import or modify the game runtime.
