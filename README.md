...
- `tools/wol-air-dash-demo.html` — a standalone, phone-first playable demo of the two Air dash Arcana in the showcase video, Gust Burst and Razor Burst, in both their base and enhanced forms. Thumbstick plus dash button on touch, WASD/space on keyboard, gamepad supported. A `SOURCE` sheet plays each Arcana's bounded showcase clip so the demo can be compared against the footage directly. Simulation lives in `src/wol-air-dash-demo.js`; the numbers are locked by `tests/wol-air-dash-demo.test.mjs`, which encodes the acceptance tests published in `archive/wizard-of-legend/source-notes/02-gust-razor.md`. It does not import or modify the game runtime.
